...
def get_or_create_sensor(sensor_nome):
    sensor, created = Sensor.objects.get_or_create(nome=sensor_nome)
    return sensor
...
def salvar_dados_no_banco(sensor_nome, estado):
    ...
    # Pega (ou cria) o sensor no banco
    sensor_obj = get_or_create_sensor(sensor_nome)
    ...
    print(f"Salvo no banco: {sensor_nome} -> Estado: {estado}, Timestamp: {timestamp}")
...
def coleta_dados():
    sensor_estado = {
        "Sensor_Silo_Superior": 3,
        "Sensor_Silo_Inferior": 3,
        "Sensor_Caixinha_Conectores": 3,
        "Sensor_Esteira": 3
    }
